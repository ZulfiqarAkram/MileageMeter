...
import com.zulfiqar.mileagemeter.databinding.FragmentVehicleSetupBinding;
import com.zulfiqar.mileagemeter.models.Vehicle;
import com.zulfiqar.mileagemeter.models.VehicleType;
import com.zulfiqar.mileagemeter.viewmodels.VehicleSetupViewModel;
...
public class VehicleSetupFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Get vehicleId from arguments
        VehicleSetupFragmentArgs args = VehicleSetupFragmentArgs.fromBundle(getArguments());
        long vehicleId = args.getVehicleId();
        
        if (vehicleId != -1L) {
            // We're editing an existing vehicle
            viewModel.loadVehicle(vehicleId);
            viewModel.getVehicle().observe(getViewLifecycleOwner(), this::populateFields);
        }
    }
    ...
    private void populateFields(Vehicle vehicle) {
        if (vehicle != null) {
            ...
            binding.fuelCapacityInput.setText(String.format(Locale.US, "%.1f", vehicle.getFuelTankCapacity()));
            binding.activeCheckbox.setChecked(vehicle.isActive());
        }
    }
    ...
    private void validateAndSaveVehicle() {
        ...
        vehicle.setActive(binding.activeCheckbox.isChecked());
